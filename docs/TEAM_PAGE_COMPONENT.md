# Team Page Component - Detailed Specification
...
```javascript
// The WhatsApp button includes:
// 1. data-whatsapp-number attribute for later configuration
// 2. Default onClick handler that opens WhatsApp Web/App
// 3. Professional green color matching WhatsApp brand

// To configure later:
// Replace '919999999999' with actual phone number in international format
// Format: countrycode + number (no + sign or spaces)
// Example: 919876543210 for India (+91 9876543210)

// Optional: Add WhatsApp Business API integration
const handleWhatsAppClick = async () => {
  // Track click event
  await fetch('/api/analytics/whatsapp-click', { method: 'POST' });
  
  // Open WhatsApp
  const phoneNumber = document.querySelector('[data-whatsapp-number]').dataset.whatsappNumber;
...
1. **Color Scheme**:
   - BNI Red: #D32F2F
   - WhatsApp Green: #25D366 (hover: #22c55e)
   - Gradient: from-red-600 to-amber-500

2. **Animations**:
   - Cards: hover:shadow-xl transition-shadow duration-300
   - Buttons: hover effects with transition-colors
   - Developer card: Special gradient border

3. **Responsive Design**:
   - Mobile: Stack all cards vertically
   - Tablet: 2 columns for team members
   - Desktop: Optimal grid layouts

4. **Typography**:
   - Headers: font-bold with appropriate sizing
   - Body: Clean, readable text with proper spacing
   - Special emphasis on developer section